    ...
    <div class="k8s-monitoring">
      ...
      <template v-else>
        ...
        <main class="monitoring-main">
          ...
          <template v-else-if="page.monitorView === 'pod'"><div class="monitoring-section-head"><div><h3>Pod 监控</h3><p>按命名空间、节点、工作负载或 Pod 收窄资源与网络趋势</p></div><span class="monitoring-section-count">{{ page.monitorPodFilteredPods.length }} 个 Pod</span></div><section class="monitoring-pod-filter-card" aria-label="Pod 监控筛选"><label><span class="monitoring-filter-tag">Namespace</span><el-select v-model="page.monitorPodNamespace" filterable size="small" @change="page.handleMonitorPodScopeChange"><el-option v-for="item in page.monitorPodNamespaceOptions" :key="item.value" :label="item.label" :value="item.value" /></el-select></label><label><span class="monitoring-filter-tag">Node</span><el-select v-model="page.monitorPodNode" filterable size="small" @change="page.handleMonitorPodScopeChange"><el-option v-for="item in page.monitorPodNodeOptions" :key="item.value" :label="item.label" :value="item.value" /></el-select></label><label><span class="monitoring-filter-tag">工作负载</span><el-select v-model="page.monitorPodWorkload" filterable size="small" @change="page.handleMonitorPodWorkloadChange"><el-option v-for="item in page.monitorPodWorkloadOptions" :key="item.value" :label="item.label" :value="item.value" /></el-select></label><label><span class="monitoring-filter-tag">Pod</span><el-select v-model="page.monitorPodName" filterable size="small"><el-option v-for="item in page.monitorPodNameOptions" :key="item.value" :label="item.label" :value="item.value" /></el-select></label></section></template>
          <template v-else><div class="monitoring-section-head"><div><h3>网络监控</h3><p>节点与 Pod 的实时流入、流出和吞吐趋势</p></div></div></template>
          ...
          <div class="monitoring-charts-grid"><article v-for="chart in page.monitorVisibleCharts" :key="chart.title" v-loading="chart.loading" :class="['monitoring-chart-card', { 'monitoring-bar-card': chart.type === 'bar' }]"><header><span>{{ chart.title }}</span><b>{{ page.monitorLatestValue(chart) }}</b></header><div v-if="chart.type === 'bar' && chart.series.length" class="monitoring-bar-canvas"><div v-for="(row, index) in page.monitorBarRows(chart)" :key="row.label" class="monitoring-bar-row"><span :title="row.label">{{ row.label }}</span><i><b :style="{ width: page.monitorBarWidth(chart, row.value), background: page.monitorColor(index) }" /></i><strong>{{ page.monitorFormatValue(row.value, chart.unit) }}</strong></div></div><div v-else-if="chart.series.length" class="monitoring-chart-canvas"><div class="monitoring-chart-scale"><span>{{ page.monitorFormatValue(chart.maxValue, chart.unit) }}</span><span>{{ page.monitorFormatValue(chart.minValue, chart.unit) }}</span></div><svg viewBox="0 0 1000 220" preserveAspectRatio="none" role="img" :aria-label="chart.title" @mousemove="page.updateMonitorChartHover(chart, $event)" @mouseleave="page.monitorHover = null"><line v-for="grid in 5" :key="grid" x1="42" x2="960" :y1="14 + (grid - 1) * 46" :y2="14 + (grid - 1) * 46" class="monitoring-chart-grid" /><path :d="page.monitorChartArea(chart, chart.series[0].values)" class="monitoring-chart-area" /><path v-for="(series, index) in chart.series" :key="series.label" :d="page.monitorChartPath(chart, series.values)" :stroke="page.monitorColor(index)" class="monitoring-chart-line" /><template v-if="page.monitorHover?.chart === chart.title"><line :x1="42 + page.monitorHover.position * 918" :x2="42 + page.monitorHover.position * 918" y1="14" y2="198" class="monitoring-chart-cursor" /></template></svg><div v-if="page.monitorHover?.chart === chart.title" class="monitoring-chart-tooltip" :style="page.monitorTooltipStyle()"><strong>{{ page.monitorTimeText(page.monitorHover.time) }}</strong><span v-for="point in page.monitorHover.points" :key="point.label"><i :style="{ background: point.color }" />{{ point.label }}<b>{{ page.monitorFormatValue(point.value, page.monitorHover.unit) }}</b></span></div><div class="monitoring-chart-axis"><span>{{ page.monitorTimeText(chart.minTime) }}</span><span>{{ page.monitorTimeText(chart.maxTime) }}</span></div><div class="monitoring-chart-legend"><span v-for="(series, index) in chart.series.slice(0, 3)" :key="series.label"><i :style="{ background: page.monitorColor(index) }" />{{ series.label }}</span><em v-if="chart.series.length > 3">+{{ chart.series.length - 3 }} 条序列</em></div></div><el-empty v-else-if="!chart.loading" :description="chart.failed ? '指标查询失败，可单独刷新重试' : '当前数据源暂无该指标'" :image-size="42" /></article></div>
        </main>
      </template>
    </div>
